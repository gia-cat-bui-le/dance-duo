...
def img_preprocess(image, input_size=512):
    ...
    input_image = torch.from_numpy(
        cv2.resize(pad_image, (input_size, input_size), interpolation=cv2.INTER_CUBIC)
    )[None].float()
    return input_image, image_pad_info


def convert_tensor2numpy(
    outputs, del_keys=["verts_camed", "smpl_face", "pj2d", "verts_camed_org"]
):
    for key in del_keys:
        if key in outputs:
            del outputs[key]
...
class ResultSaver:
    ...
    def __call__(self, outputs, input_path, prefix=None, img_ext=".png"):
        ...
        if prefix is not None:
            save_path = (
                osp.splitext(save_path)[0] + f"_{prefix}" + osp.splitext(save_path)[1]
            )
        ...
        if self.mode == "video":
            # print("saver self.mode = video")
            self.frame_save_paths.append(save_path)

    def save_video(self, save_path, frame_rate=24):
        if len(self.frame_save_paths) == 0:
            return
        self.frame_save_paths.sort()
        # print(self.frame_save_paths)
        # print(type(self.frame_save_paths))
        height, width = cv2.imread(self.frame_save_paths[0]).shape[:2]
        writer = cv2.VideoWriter(
            save_path, cv2.VideoWriter_fourcc(*"mp4v"), frame_rate, (width, height)
        )
...
def save_video_results(frame_save_paths):
    ...
    video_results_save_path = osp.join(
        osp.dirname(frame_save_paths[0]), "video_results.npz"
    )
    np.savez(
        video_results_save_path,
...
def collect_frame_path(video_path, save_path):
    ...
    if osp.isfile(video_path):
        ...
        try:
            ...
            raise Exception(
                f"Failed in extracting the frames of {video_path} to {frame_save_dir}! \
    ...
    assert osp.isdir(frame_save_dir), (
        frame_save_dir + "is supposed to be a folder containing video frames."
    )
    frame_paths = [
        osp.join(frame_save_dir, frame_name)
...
def create_OneEuroFilter(smooth_coeff):
    return {
        "smpl_thetas": OneEuroFilter(smooth_coeff, 0.7),
        "cam": OneEuroFilter(1.6, 0.7),
        "smpl_betas": OneEuroFilter(0.6, 0.7),
        "global_rot": OneEuroFilter(smooth_coeff, 0.7),
    }
...
def get_tracked_ids(detections, tracked_objects):
    ...
    tracked_ids = [
        tracked_ids_out[np.argmin(np.linalg.norm(tracked_points - point[None], axis=1))]
...
def get_tracked_ids3D(detections, tracked_objects):
    ...
    tracked_ids = [
        tracked_ids_out[
            np.argmin(
                np.linalg.norm(
                    tracked_points.reshape(-1, 4) - point.reshape(1, 4), axis=1
                )
            )
        ]
        for point in org_points
...
def vertices_kp3d_projection(outputs, meta_data=None, presp=False):
    ...
    cam_trans = estimate_translation(
        predicts_j3ds, predicts_pj2ds, focal_length=443.4, img_size=np.array([512, 512])
    ).to(vertices.device)
    projected_outputs = {
...
def transform_rot_representation(
    rot, input_type="mat", out_type="quat", input_is_degrees=True
):
    """
...
def download_model(remote_url, local_path, name):
    try:
        ...
        print(error)
        print(
            "Failure in downloading the {} model, please download it by youself from {}, and put it to {}".format(
                name, remote_url, local_path
            )
        )
